...
ResultSet r1;
      Statement st1=conn.createStatement();
      String sqlcount="select count(product_id) as totalproducts from master_product where product_id='"+this.productId+"'";
      r1=st1.executeQuery(sqlcount);
 ...
 public String[][] getProductInfoBySearch(String productId) throws SQLException
    {
     ...
     String[][] productList=new String[rowscount][4];
      Statement st1=conn.createStatement();
     String sqlselect="select product_id,product_name,Product_size,product_price from master_product where product_id ='"+this.productId+"' order by product_id";
     ResultSet rs=st1.executeQuery(sqlselect);
     int row=0,col=0;
